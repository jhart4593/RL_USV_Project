...
from utilities.utils import get_obs, get_obs_norm, PID, LOS_guidance, get_current_LOS
...
class USVEnv_eval_LOS(gym.Env):
    ...
    def __init__(
        ...
        get_current: Callable = get_current_LOS,
    ...
    def reset(self, seed=None, options=None):
        ...
        self.simData = np.array(
            ...
            + [self.t] + [self.yaw_error,self.path_error] + [self.tau_X,self.tau_N] + [0,0]
        )
    ...
    def step(self, action):
        ...
        [self.eta,self.nu,self.u_actual,self.nu_dot,power] = (
        ...
        self.simData = np.vstack(
            (self.simData,np.array(list(self.eta) + list(self.nu) + list(self.u_control) 
                ...
                + [self.tau_X,self.tau_N] + list(power)))
            )
